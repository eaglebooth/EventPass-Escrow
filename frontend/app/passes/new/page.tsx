...
export default function NewPassPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      const hash = await writeContract("create_listing", [form.event, form.seat, form.policy, normalizeDigest(form.digest), terms, bond], bond);
  ...
  return <main><SiteHeader /><ContractStrip /><section className="composer-page">
    ...
    <div className="composer-layout">
      ...
      <section className="composer-console"><div className="console-heading"><ShieldCheck size={21} /><div><span>Listing specification</span><b>Public terms and custody</b></div></div>
        <div className="form-grid"><label>Event name<input value={form.event} onChange={(e) => update("event", e.target.value)} /></label><label>Seat / tier<input value={form.seat} onChange={(e) => update("seat", e.target.value)} /></label><label>Official policy URL<input value={form.policy} onChange={(e) => update("policy", e.target.value)} placeholder="https://..." /></label><label>Policy SHA-256<input value={form.digest} onChange={(e) => update("digest", e.target.value)} placeholder="sha256: + 64 lowercase hexadecimal characters" /></label><label>Sale price (GEN)<input value={form.price} onChange={(e) => update("price", e.target.value)} /></label><label>Seller bond (GEN)<input value={form.bond} onChange={(e) => update("bond", e.target.value)} /></label></div>
